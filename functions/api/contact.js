// Cloudflare Pages Function: POST /api/contact (Windstorm Labs site)
...
export async function onRequestPost({ request, env }) {
  ...
  const subjectFinal = `[Labs] ${subject.slice(0, 140)}`;
  const textBody =
`Contact form submission from windstormlabs.com
...
<body style="font-family:-apple-system,BlinkMacSystemFont,Inter,sans-serif;font-size:15px;line-height:1.55;color:#1a1a18;max-width:640px;margin:0 auto;padding:24px;">
  <p style="font-family:monospace;font-size:11px;color:#6b6b67;letter-spacing:0.1em;text-transform:uppercase;margin:0 0 16px;">Contact form &middot; windstormlabs.com</p>
  <p style="margin:0 0 8px;"><strong>From:</strong> ${esc(name)} &lt;<a href="mailto:${esc(email)}" style="color:#0f7a3c;">${esc(email)}</a>&gt;</p>
